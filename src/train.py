...
class DataCollatorCausalLMWithLabels:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for f in features:
            ...
            labels.append(lab)
        return {'input_ids':batch_pad['input_ids'], 'attention_mask':batch_pad['attention_mask'], 'labels':torch.stack(labels)}
...
def main():
    ...
    train_ds = tokenized['train']; val_ds = tokenized['validation']
    collator = DataCollatorCausalLMWithLabels(tokenizer)
    train_loader = DataLoader(train_ds, batch_size=args.per_device_batch_size, shuffle=True, collate_fn=collator, num_workers=args.num_workers, pin_memory=True)
    val_loader   = DataLoader(val_ds, batch_size=args.per_device_batch_size, shuffle=False, collate_fn=collator, num_workers=args.num_workers, pin_memory=True)
    ...
    for epoch in range(args.epochs):
        ...
        model.eval(); eval_loss=0.0; nb=0
        with torch.no_grad():
            for batch in val_loader:
                outputs=model(**batch); eval_loss+=outputs.loss.item()*batch['input_ids'].shape[0]; nb+=batch['input_ids'].shape[0]
        eval_loss/=max(1,nb); import math; ppl=math.exp(eval_loss) if eval_loss<30 else float('inf')
        if accelerator.is_main_process:
            print(f"[Epoch {epoch}] eval_loss={eval_loss:.4f} perplexity={ppl:.2f}")
            unwrapped = accelerator.unwrap_model(model); unwrapped.save_pretrained(args.output_dir); tokenizer.save_pretrained(args.output_dir)
    if accelerator.is_main_process:
